...
size_t lomuto_partition(int array[], size_t left, size_t right) {
    ...
    for(size_t i = left+1; i<=right; i++) 
	{
    	...
    	{
    		iter++;
    		swap(array[iter], array[i]);
    		
		}
	}
	swap(array[iter], array[left]);
    return iter;
}
...
int main(int argc, char *argv[]) {
	...
	if(k > int(size))
	{
		cerr << "Error: Cannot find smallest element " << k << " with only " << size << ( (size==1) ? " value." : " values.") << endl;
		return 1;
	}
